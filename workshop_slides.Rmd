...
Familiarize wth the number of checks (see http://r-pkgs.had.co.nz/check.html for reference)
...
- Create release branch `release/vX.Y.Z` from `develop`, where the package version includes a fourth development component `9000`.
- Update the package version via `usethis::use_version()` locally on branch `release/v1.1.0`.
...
- Create release on GitHub as "v1.1.0", with title "ShinyCICD 1.1.0" and content of the corresponding `NEWS.md` section.
