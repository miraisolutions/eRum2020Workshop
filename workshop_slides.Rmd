...
## Mirai Solutions contributions
- Involved in organizing eRum 2020
- Eager contributors to open source projects

```{r eval = TRUE, echo = FALSE, out.width = "55%", out.extra='style="position:absolute;bottom:80px;right:50px;"'}
...
It is good practice to integrate a Shiny app in a package, as then one can take full advantage of all the integrated features already established for an R package (e.g. automatic testing, `R CMD check`, etc.).
...
Git is a popular, free and open source version control system, designed to be very efficient and to support distributed, non-linear workflows. With GitHub hosting, it is the _de facto_ standard for open source (R) projects
...
- `R CMD check` built into R (and conveniently accessed via `devtools::check()`, `rcmdcheck::rcmdcheck()`)
...
- Look at the R console and the Git diffs: what files were changed by `usethis::use_travis()`? (don't commit changes yet)
...
- Go to the GitHub repo
  - You can use `usethis::browse_github()`
...
- Future runs will be much faster thanks to caching caching. 
